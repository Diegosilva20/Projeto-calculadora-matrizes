...
function Home() {
  ...
  const calculate = () => {
    ...
    if (operation === "soma") {
      ...
      const resultMatrix = Array.from({ length: size }, () =>
        Array(size).fill(0)
      );
  ...
  return (
    <div className="p-6 text-center">
      ...
      <p className="mb-4">
        Realize operações como soma, subtração e determinante de matrizes.
      </p>
          ...
          >
            {result.map((row, i) =>
              row.map((val, j) => (
                <div
                  key={`result-${i}-${j}`}
                  className="border p-2 rounded w-16 text-center bg-gray-200"
                >
                  {val}
                </div>
              ))
...
function PoliticaPrivacidade() {
  return (
    <div className="p-6">
      ...
      <p>
        Este site utiliza cookies e serviços de terceiros como o Google AdSense.
      </p>
    </div>
  );
}
