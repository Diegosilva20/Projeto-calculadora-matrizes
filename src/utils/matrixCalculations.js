...
export const createEmptyMatrix = (n) =>
    ...
    if (operation === "soma") {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (Math.abs(pivot - 1) > 1e-10) {
          steps.push({
            description: `Tornar o pivô [${i + 1},${i + 1}] igual a 1: Dividir a Linha ${i + 1} por ${pivot.toFixed(
              2
            )} (pivô).`,
            matrix: null,
          });
        ...
        for (let k = i + 1; k < n; k++) {
          ...
          steps.push({
            description: `Zerar o elemento [${k + 1},${i + 1}]: Subtrair ${factor.toFixed(
              2
            )} × Linha ${i + 1} da Linha ${k + 1}.`,
            matrix: null,
          });
          for (let j = i; j < n; j++) {
